function initUnitFilter() {
    if (window.unitFilterRegistered) return;
    ...
    Alpine.data("unitFilterSystem", () => ({
        ...
        init() {
            this.$nextTick(() => {
                if (window.Isotope) {
                    this.iso = new Isotope(".unit-grid", {
                        itemSelector: ".unit-item",
                        layoutMode: "fitRows",
                        percentPosition: true
                    });

                    imagesLoaded(".unit-grid", () => {
                        this.iso.layout();
                    });
                }
            });
    ...
    console.log("Unit Filter System Successfully Registered.");
}

document.addEventListener("alpine:init", initUnitFilter);
